...
- 日常更新走部署链路自动同步：`deploy/update.sh` / `deploy/full-deploy.sh` 统一调用 `deploy/sync-qwenpaw-skills.sh`（经 `docker exec qwenpaw` 跑 `quantbot_init`，避免 Web updater 容器打不到宿主 8088）；`--skip-skills` / `QUANTMIND_SKIP_SKILLS=true` 可跳过
