...
<p align="center">
  <strong>AI 原生 · 13 种模型工场 · 因子自主进化 · 工业级多市场量化投研平台</strong>
</p>
...
> ## ⚠️ 使用前必读
>
> **本项目是量化研究与回测工具，不是投资顾问服务，不构成任何投资建议。**
> 它不推荐你买卖任何具体证券、不代客理财、不接触你的资金、不提供任何收益承诺。
> 模型输出一律表达为中性的**研究评分**（0–100 截面分位）与**截面位置**（靠前/靠后/居中），
> 请勿把它们读成买卖指令。
>
> **实盘相关功能在开源版中默认关闭**（`ENABLE_REAL_TRADING` / `VITE_ENABLE_REAL_TRADING`
> 均需显式开启）。开启实盘即意味着你自行承担全部交易风险，并自行履行**程序化交易报告义务**
> （含向券商申报交易软件名称/版本号/开发者，以及可能的高频交易额外报告）。
>
> 完整条款（输出口径、数据与模型局限、程序化交易合规、责任限制）见
> **[DISCLAIMER.md](DISCLAIMER.md)**。**股市有风险，投资需谨慎。**
...
```text
数据底座 -> 因子挖掘 -> 模型训练 -> 批量推理 -> 组合回测 -> 模拟盘演练 -> 生产监控
```
...
| **QMT 通道**（默认关闭）   | • 原生对接券商 **MiniQMT (xtquant)**，配备 Windows 独立 QMT Agent 桌面客户端 • 采用**加密 WebSocket Bridge** 双向通信，支持同步/异步买卖、保护限价单与撤单防饥饿队列 • 账户资产与持仓秒级同步落库，完整支持柜台异步成交回报与断线看门狗自动重连 • ⚠️ **开源版默认关闭**，须显式开启 `ENABLE_REAL_TRADING`；启用者自行承担全部交易风险并自行履行程序化交易报告义务                                  |
| **通达信深度联动**    | • 模型截面选股结果**推入通达信自定义板块** • 盘中实时预警雷达弹窗 + **双击下单**（模拟盘；实盘通道默认关闭）                                                                                                                             |
...
>
> * 本项目是**量化研究与回测工具**，不提供投资顾问、资产管理或任何其他需要持牌的服务；
>   不推荐具体证券的买卖时机，不代客理财，不提供账户共用或子账户服务，不作任何收益承诺；
>
> * 本系统产出的所有分析报告与信号均由算法自动生成，一律以中性的研究评分（0–100 截面分位）
>   与截面位置（靠前/靠后/居中）表达，**跨模型、跨市场、跨交易日不可直接比较**，可能存在误差或失效风险；
>
> * 回测结果受幸存者偏差、前视偏差、过拟合、交易成本与滑点假设影响，与实盘结果可能存在重大差异，
>   **历史数据与回测结果不代表未来收益**；
>
> * **实盘功能默认关闭**；启用者须自行履行程序化交易报告义务（含高频交易的额外报告）并自担全部交易风险；
>
> * 实际投资决策请结合自身风险承受能力或咨询持有中国证监会颁发资质的专业机构；
>
> * 在适用法律允许的最大范围内，作者与贡献者不对使用本开源软件产生的任何投资损失或其他损失承担责任；
>
> * **股市有风险，入市需谨慎。**
>
