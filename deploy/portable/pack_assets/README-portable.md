...
**训练数据 = 直读各市场因子（`data/<市场根>/6_ml_datasets/`，dt=YYYYMMDD 分区）**：
A股=`data/quantdb/`、港股=`data/quanthk/`、美股=`data/quantus/`。数据就绪两种方式：
① 从离线数据包/网盘把对应市场目录放进 `data/` 下；② 在「实时数据流 → 数据管理」
页面同步因子（需在 `pack.env` 配好相应平台的访问凭据）。训练时在数据源选择
「直读因子」并选数据集（l1/l2/l1_l2…，新增数据集自动识别，无需更新包）。

**Windows 一键训练自检（首次使用）**：
1. `start.bat` 启动 → 进「训练中心」，节点应显示「本地直跑(免Docker)」且就绪
2. 选 A股 + 直读因子 + 年份区间 → 开始训练；日志实时可见，产物自动注册
3. 训练期间 CUDA 检测：装了 GPU 增补包则训练页节点显示 GPU 型号，DL 模型自动用 GPU
4. 训练数据缺分区时先做上面「数据就绪」任一步骤，训练会按所选区间读因子分区
...
- **同步更新代码后启动失败**：sync 脚本覆盖前会自动把当前可运行代码备份到包内 `backups/`，此时运行包根 `restore_backup.bat`（Windows）或 `./restore_backup.sh`（Linux/WSL）即可恢复同步前的状态（含前端 web；旧备份无 web 时保留现有前端），再重新启动。
