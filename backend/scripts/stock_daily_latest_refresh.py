"""滚动刷新 PG stock_daily_latest 最近 N 天（默认 30 天 ≈ 1 个月），数据源 QuantDB。

被 `quantdb_daily_sync.run_daily_sync` 的 PG 阶段调用——**随 QuantDB 同步一起跑**，
QuantDB 没同步就不会执行；也可由 `scripts/data/maintenance/rolling_sync_stock_daily_recent.py`
手动执行。

口径与市场定时同步完全一致：`fill_pg_from_parquet`（前复权 OHLCV + adj_factor=1.0 +
估值/波动/收益特征 + 价格派生指标），随后富化 fill 不写的列（stock_name/industry/
is_st/idx_*/concept_*/roe/bp/ep_ttm/ln_mv_total/turnover_rate/listed_days）。
upsert 幂等，可重复运行。
"""
...
LOGGER = logging.getLogger("stock_daily_latest_refresh")
...
def _quantdb_latest_trade_date() -> date | None:
...
def refresh_stock_daily_latest(
    days: int = 30,
    *,
    ...
    """滚动刷新 stock_daily_latest 最近 N 天（QuantDB 源）。

    force=False 时带「QuantDB 未推进则跳过」门控（表内最新日 >= QuantDB 最新分区日
    即退出），使随 QuantDB 同步调用时不会空跑。
    """
    ...
    rows_before, min_before, max_before = _table_range(url)
    LOGGER.info(
        "stock_daily_latest 刷新前: rows=%s range=[%s, %s]；窗口=[%s, %s]",
        rows_before,
        min_before,
        max_before,
        start_date,
    ...
    if not force:
        qdb_latest = _quantdb_latest_trade_date()
        if qdb_latest is not None and max_before is not None and max_before >= qdb_latest:
            LOGGER.info(
                "QuantDB 未推进（QuantDB 最新=%s，表内最新=%s），跳过", qdb_latest, max_before
            )
            return {"status": "skipped", "reason": "quantdb_not_advanced"}
    ...
    try:
        _enrich_names_industry(url, start_date)
    except Exception as exc:  # noqa: BLE001
        LOGGER.warning("名称/行业富化失败（不阻断）: %s", exc)

    try:
        _enrich_gap_columns(url, start_date, end_date)
    except Exception as exc:  # noqa: BLE001
        LOGGER.warning("缺口列富化失败（不阻断）: %s", exc)

    if prune:
        deleted = _prune(url, start_date)
        LOGGER.info("prune：删除窗口外行 %s 条（< %s）", deleted, start_date)

    _invalidate_cache()

    rows_after, min_after, max_after = _table_range(url)
    LOGGER.info("stock_daily_latest 刷新后: rows=%s range=[%s, %s]", rows_after, min_after, max_after)

    result["status_after"] = result.get("status")
    result["table_range_after"] = (
        str(min_after) if min_after else None,
        str(max_after) if max_after else None,
    )
    return result
