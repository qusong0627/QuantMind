...
⚠️ **风格集扩容会改掉旧风格的纯因子收益**（WLS 的解释变量集变了）。10 → 12 的实测
断点（2026-09-22）：旧十风格 ρ=0.663~0.970（earningsyield 最低、liquidity 最高），
earningsyield 平均 |Δ|=0.061%/日 ≈ 0.54σ。故**跨扩容前后的历史报告数字不可直接比**，
报告归档时以 ``meta.json`` 的风格清单为准。同日双披露修复（``pit_order``）的断点
则是 ρ=1.000、|Δ|≤0.0009%/日 —— 那是修 bug、不动口径。
