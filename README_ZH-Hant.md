...
QuantMind 是**量化研究與回測工具**，僅供研究與學習使用，**不構成任何投資建議**，
亦不提供投資顧問、資產管理或任何其他需要持牌之服務，不推薦任何特定證券之買賣時機，
不代客理財，不作任何收益承諾。模型輸出一律以中性的**研究評分**（0–100 截面分位）與
**截面位置**（靠前／靠後／居中）表達，請勿解讀為買賣指令；跨模型、跨市場、跨交易日
不可直接比較。

**實盤相關功能在開源版預設關閉**（`ENABLE_REAL_TRADING` / `VITE_ENABLE_REAL_TRADING`
均須顯式開啟）。啟用者須自行承擔全部交易風險，並自行履行程序化交易報告義務
（含向券商申報交易軟體名稱、版本號、開發者，以及可能之高頻交易額外報告）。

完整條款見 **[DISCLAIMER.md](DISCLAIMER.md)**。**股市有風險，投資需謹慎。**
