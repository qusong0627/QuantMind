...
- predict：统一预测接口（WFA 与主流程共用）。

聚合导入保证：任何 `import model_trainers[.x]` 都触发完整注册
（GBDT 装饰器 + DL 循环），MODEL_REGISTRY 恒为全集。
"""

from model_trainers import metrics, predict, registry, trainers_dl, trainers_gbdt

__all__ = ["metrics", "predict", "registry", "trainers_dl", "trainers_gbdt"]
