...
| P2 执行统一 | 4/7（新增 T-P2-07 盘后固定价格窗口） | 回测-模拟一致性 diff=0 ✅（v1 执行层） |
...
- **T-P2-05b（记录待决）**：① **策略层回放平价**（同策略对象驱动回测 vs 时光回放，真实历史数据）；
  ② **顺延 vs 拒单语义统一**——回测对不可成交单挂单顺延、模拟立即拒单（当前夹具只断言"两测均不成交"）；
  建议按订单类型统一：市价单当日不顺延（拒）、限价单可顺延（挂单）。
