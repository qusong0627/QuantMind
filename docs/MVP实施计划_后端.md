...
> - **本批不做（记录在案）**：~~回测 T+1 锁定~~ → **已收口（2026-09-16 深夜，见下）**、
...
**附注（2026-09-16 深夜核实关闭）**：申报数量新规的科创板部分已在 T-P2-02 落地；
**创业板"200 股起"经官方规则原文核实为误传——创业板=100 股整数倍（与主板一致），
"200 股起 1 股递增"是科创板专属**；代码现有口径（创业板 100 整数倍）**正确，无需改动**。
核对中同时核实 2026-07-06 新规的另两条口径并收口：
① **单笔申报数量上限**（原实现缺失）——主板/创业板 限价≤30 万·市价≤15 万，科创板 限价≤10 万·市价≤5 万，
盘后固定价格（全 A 股/ETF）≤100 万股；`market_rules.order_quantity_cap` 唯一实现，
模拟 execution_engine（含会话判定）与回测引擎双侧接线（保平价），超限拒单留因；
② **盘后申报价约束**（买申报不得低于收盘价、卖不得高于收盘价）——模拟限价分支"按市价成交/买<市价拒"
语义即该规则本身（构造上不可能出范围价），维持零改动（T-P2-07 已记录）。
...
### T-P2-02 记录在案项收口：回测引擎 T+1 锁 + 单笔申报上限（2026-09-16 深夜）
- **回测 T+1 锁**（portfolio 账本模型）：`Position.available_quantity`（当日买入次日才解锁）+
  `Portfolio.unlock_t1()`（引擎日循环起始、先于当日成交处理）+ sell 按可卖量校验 +
  `_process_orders` 捕获业务性拒单（ValueError→REJECTED 留因，不炸整轮回测）——与模拟盘
  lua available_volume / 回放 unlock_t1 **同一语义**。实测：同日回转（同一决策日买+卖同标）
  卖出被拒 `T+1 不可卖`；跨日卖出正常；策略层平价（05c）复跑仍逐笔 diff=0。
  复盘结论：既有"买→次日管线成交"天然晚一天，T+0 违规只可能出现在**同日买+卖同标**的
  排队场景——新模型把该场景从静默成交换成显式拒单。
- **单笔申报上限**（见 T-P2-07 附注核实）：`market_rules.order_quantity_cap` 唯一实现 +
  模拟/回测双接线；测试 `test_order_quantity_caps.py` 3/3（表/回测 E2E/源守卫）。
- 测试：`test_backtest_t1_lock.py` 4/4；受影响两处旧夹具按真实语义补 `unlock_t1`
  （费用平价与账本对账夹具，断言零改动）。
