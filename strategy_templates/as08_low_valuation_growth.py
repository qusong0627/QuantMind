# -*- coding: utf-8 -*-
"""A股低估值成长 (as08_low_valuation_growth)

[A股] PE<40 / PS<8 但要求营收 10 亿与净利润 1 亿，寻找估值尚可的成长股。
...
回测记录（模型 mdl_cn_train_20260906023306_57ce74a7_d5a3faa7，2024-01-02 → 2024-12-31，A股费率/T+1/含交易成本）
    年化 43.03% ｜ 夏普 1.892 ｜ 最大回撤 -12.31% ｜ 基准 16.20% ｜ 交易 950 笔 ｜ 胜率 53.16%
    对照 standard_topk（平台内置，同模型同区间）：年化 59.26% ｜ 夏普 1.918 ｜ 最大回撤 -18.42%
    注：单一区间单一模型的成绩只用于验证链路与相对比较，不等于未来收益。

怎么用
    1. AI-IDE → 策略模板 → 文件夹「A股策略/02_价值与质量」→ 选「A股低估值成长」，选好模型直接回测；
    2. 参数面板可调 topk / n_drop / rebalance_days；f_* 是 A 股硬约束（基本面/流动性），建议保留；
    3. 命令行单跑（下面就是复现本文件回测记录的命令）：
       docker exec quantmind python /app/scripts/verify_ashare_backtest.py as08_low_valuation_growth \
         --start 2024-01-02 --end 2024-12-31 --model-id mdl_cn_train_20260906023306_57ce74a7_d5a3faa7
    4. 实盘：策略 ID 用 sys_as08_low_valuation_growth（内置模板在实盘链路里需加 sys_ 前缀）。
...
STRATEGY_CONFIG = {
    ...
    "kwargs": {
        ...
        "f_pe_ttm_max": 40,
        "f_ps_ttm_max": 8.0,
        "f_revenue_ttm_min": 1000000000.0,
        "f_net_profit_ttm_min": 100000000.0,
