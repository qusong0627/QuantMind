# -*- coding: utf-8 -*-
"""A股恐慌低吸 (as44_panic_reversal)

[A股] 上一交易日跌幅 2%-9%、中期趋势仍向上（站上 20 日线）、RSI<55 的单日恐慌错杀。

文件夹：A股策略/09_事件与择时 ｜ 策略类：RedisRecordingStrategy ｜ 市场：A股
调仓：每 3 个交易日 ｜ 持仓：25 只 ｜ 单期换手：12 只

回测记录（模型 mdl_cn_train_20260906023306_57ce74a7_d5a3faa7，2024-01-02 → 2024-12-31，A股费率/T+1/含交易成本）
    年化 10.19% ｜ 夏普 0.282 ｜ 最大回撤 -23.27% ｜ 基准 16.20% ｜ 交易 1847 笔 ｜ 胜率 52.72%
    对照 standard_topk（平台内置，同模型同区间）：年化 59.26% ｜ 夏普 1.918 ｜ 最大回撤 -18.42%
    注：单一区间单一模型的成绩只用于验证链路与相对比较，不等于未来收益。

怎么用
    1. AI-IDE → 策略模板 → 文件夹「A股策略/09_事件与择时」→ 选「A股恐慌低吸」，选好模型直接回测；
    2. 参数面板可调 topk / n_drop / rebalance_days；f_* 是 A 股硬约束（基本面/流动性），建议保留；
    3. 命令行单跑（下面就是复现本文件回测记录的命令）：
       docker exec quantmind python /app/scripts/verify_ashare_backtest.py as44_panic_reversal \
         --start 2024-01-02 --end 2024-12-31 --model-id mdl_cn_train_20260906023306_57ce74a7_d5a3faa7
    4. 实盘：策略 ID 用 sys_as44_panic_reversal（内置模板在实盘链路里需加 sys_ 前缀）。
...
STRATEGY_CONFIG = {
    ...
    "kwargs": {
        ...
        "n_drop": 12,
        "rebalance_days": 3,
        "f_pct_change_min": -9.0,
        "f_pct_change_max": -2.0,
        "f_ma_gap_20_min": 0.0,
        "f_rsi_14_max": 55,
