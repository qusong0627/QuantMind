# -*- coding: utf-8 -*-
"""A股反转+行业分散护栏 (as24_reversal_guard)

[A股] 反转选股叠加行业持仓上限与大盘状态降仓，避免在单一行业深跌里越买越多。

文件夹：A股策略/05_反转与均值回归 ｜ 策略类：RedisRiskGuardTopkStrategy ｜ 市场：A股
调仓：每 3 个交易日 ｜ 持仓：30 只 ｜ 单期换手：10 只

回测记录（模型 mdl_cn_train_20260906023306_57ce74a7_d5a3faa7，2024-01-02 → 2024-12-31，A股费率/T+1/含交易成本）
    年化 35.48% ｜ 夏普 1.170 ｜ 最大回撤 -18.81% ｜ 基准 16.20% ｜ 交易 1532 笔 ｜ 胜率 51.88%
    对照 standard_topk（平台内置，同模型同区间）：年化 59.26% ｜ 夏普 1.918 ｜ 最大回撤 -18.42%
    注：单一区间单一模型的成绩只用于验证链路与相对比较，不等于未来收益。

怎么用
    1. AI-IDE → 策略模板 → 文件夹「A股策略/05_反转与均值回归」→ 选「A股反转+行业分散护栏」，选好模型直接回测；
    2. 参数面板可调 topk / n_drop / rebalance_days；f_* 是 A 股硬约束（基本面/流动性），建议保留；
    3. 命令行单跑（下面就是复现本文件回测记录的命令）：
       docker exec quantmind python /app/scripts/verify_ashare_backtest.py as24_reversal_guard \
         --start 2024-01-02 --end 2024-12-31 --model-id mdl_cn_train_20260906023306_57ce74a7_d5a3faa7
    4. 实盘：策略 ID 用 sys_as24_reversal_guard（内置模板在实盘链路里需加 sys_ 前缀）。
