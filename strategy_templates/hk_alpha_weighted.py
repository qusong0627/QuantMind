# -*- coding: utf-8 -*-
"""港股 Alpha 加权 (HK Alpha-Weighted)

[港股] 时间 9:30–16:00（午休 12:00–13:00）、T+0 回转、无涨跌停 ｜ 信号 <PRED> 为平台港股模型预测分（T 日收盘生成、T+1 生效）。

核心逻辑：高分散加权 Top 50、单票 ≤4%，降低个股特异性风险。
调仓：每 3 个交易日 ｜ 持仓：50 只 / 单票 ≤4% ｜ 建议调仓时点：尾盘 15:50 卖出 / 15:58 买入（json live_defaults 预填）。
参数：见同名 .json（topk / min_score / max_weight）。
使用：AI-IDE → 策略模板 → 港股 选择本模板绑定港股模型回测；实盘策略 ID 用 sys_hk_alpha_weighted。
...
STRATEGY_CONFIG = {
    ...
    "kwargs": {"signal": "<PRED>", "topk": 50, "min_score": 0.0, "max_weight": 0.04, "rebalance_days": 3},
}
