# -*- coding: utf-8 -*-
"""A股大盘风控核心 (as39_risk_guard_core)

[A股] 平台内置风控类：Beta<1.3、波动率<4%、行业上限 30%，大盘下行时自动降仓。

文件夹：A股策略/08_风险控制与仓位 ｜ 策略类：RedisRiskGuardTopkStrategy ｜ 市场：A股
调仓：每 5 个交易日 ｜ 持仓：50 只 ｜ 单期换手：10 只

回测记录（模型 mdl_cn_train_20260906023306_57ce74a7_d5a3faa7，2024-01-02 → 2024-12-31，A股费率/T+1/含交易成本）
    年化 38.11% ｜ 夏普 1.900 ｜ 最大回撤 -10.41% ｜ 基准 16.20% ｜ 交易 990 笔 ｜ 胜率 54.01%
    对照 standard_topk（平台内置，同模型同区间）：年化 59.26% ｜ 夏普 1.918 ｜ 最大回撤 -18.42%
    注：单一区间单一模型的成绩只用于验证链路与相对比较，不等于未来收益。

怎么用
    1. AI-IDE → 策略模板 → 文件夹「A股策略/08_风险控制与仓位」→ 选「A股大盘风控核心」，选好模型直接回测；
    2. 参数面板可调 topk / n_drop / rebalance_days；f_* 是 A 股硬约束（基本面/流动性），建议保留；
    3. 命令行单跑（下面就是复现本文件回测记录的命令）：
       docker exec quantmind python /app/scripts/verify_ashare_backtest.py as39_risk_guard_core \
         --start 2024-01-02 --end 2024-12-31 --model-id mdl_cn_train_20260906023306_57ce74a7_d5a3faa7
    4. 实盘：策略 ID 用 sys_as39_risk_guard_core（内置模板在实盘链路里需加 sys_ 前缀）。
