# -*- coding: utf-8 -*-
"""A股双周期动量 (as20_dual_momentum)

[A股] 60 日长周期动量 + 模型分（两个信号源），持仓 50 只、10 日调仓，追求低换手的趋势收益。

文件夹：A股策略/04_动量与趋势 ｜ 策略类：FilteredMomentumStrategy（模板内自定义，继承 RedisRecordingStrategy） ｜ 市场：A股
调仓：每 10 个交易日 ｜ 持仓：50 只 ｜ 单期换手：10 只

回测记录（模型 mdl_cn_train_20260906023306_57ce74a7_d5a3faa7，2024-01-02 → 2024-12-31，A股费率/T+1/含交易成本）
    年化 29.49% ｜ 夏普 1.500 ｜ 最大回撤 -10.97% ｜ 基准 16.20% ｜ 交易 510 笔 ｜ 胜率 56.03%
    对照 standard_topk（平台内置，同模型同区间）：年化 59.26% ｜ 夏普 1.918 ｜ 最大回撤 -18.42%
    注：单一区间单一模型的成绩只用于验证链路与相对比较，不等于未来收益。

怎么用
    1. AI-IDE → 策略模板 → 文件夹「A股策略/04_动量与趋势」→ 选「A股双周期动量」，选好模型直接回测；
    2. 参数面板可调 topk / n_drop / rebalance_days；f_* 是 A 股硬约束（基本面/流动性），建议保留；
    3. 命令行单跑（下面就是复现本文件回测记录的命令）：
       docker exec quantmind python /app/scripts/verify_ashare_backtest.py as20_dual_momentum \
         --start 2024-01-02 --end 2024-12-31 --model-id mdl_cn_train_20260906023306_57ce74a7_d5a3faa7
    4. 实盘：策略 ID 用 sys_as20_dual_momentum（内置模板在实盘链路里需加 sys_ 前缀）。
