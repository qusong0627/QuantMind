...
const PromptLibraryModal: React.FC<PromptLibraryModalProps> = ({ open, onClose }) => {
  ...
  return (
    <Modal
      ...
      zIndex={1200}
      destroyOnHidden
      classNames={{ wrapper: 'qb-prompt-modal' }}
      styles={{
        content: {
          padding: 0,
          borderRadius: 24,
    ...
    >
      <div className="flex flex-col" style={{ height: 'min(78vh, 820px)' }}>
        {/* 弹窗头部 */}
        <div className="flex items-center gap-3 px-5 h-14 shrink-0 border-b border-slate-100 bg-gradient-to-r from-slate-50/80 via-white to-white">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center shadow-md shrink-0">
            <BookMarked className="w-4 h-4 text-white" />
          </div>
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="text-[15px] font-bold text-slate-800 tracking-tight">提示词库</span>
            <span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 px-1.5 py-0.5 rounded-md">
              {filtered.length}/{PROMPT_LIBRARY_TOTAL} 条
            </span>
            <span className="hidden md:inline-flex items-center gap-1.5 text-[11px] text-slate-400">
              <span className="h-1.5 w-1.5 rounded-full bg-violet-500" />
              <span>示例 {EXAMPLE_TOTAL}</span>
              <span className="mx-0.5 text-slate-200">·</span>
              <span className="h-1.5 w-1.5 rounded-full bg-blue-500" />
              <span>模板 {TEMPLATE_TOTAL}</span>
            </span>
          </div>
          <span className="ml-auto hidden lg:block text-[11px] text-slate-400">
            复制后粘贴到下方 QuantBot 对话框即用
          </span>
          <button
            type="button"
            onClick={onClose}
            className="ml-3 flex w-7 h-7 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors shrink-0"
            title="关闭"
          >
            <X className="w-4 h-4" />
          </button>
