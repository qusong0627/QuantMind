...
export const FloatingNavBar: React.FC<FloatingNavBarProps> = ({ current, onChange }) => {
  ...
  // 实盘节点形态（Windows 单机包）没有底部导航：整个界面就是实盘交易一栏，
  // QuantBot 是那一栏侧栏里「设置」下面的一条追加页签。
  // 这里返回 null 而不是渲染空壳 —— .bottom-dock 是 absolute 覆盖层，留个空壳
  // 照样吞掉页面底部的元素（免责横条·表格最后一行）。
  if (LIVE_NODE_ONLY) return null;
